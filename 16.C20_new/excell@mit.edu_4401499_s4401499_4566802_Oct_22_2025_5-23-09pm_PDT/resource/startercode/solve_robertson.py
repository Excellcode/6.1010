...
def test_calc_all():
    ...
    p = {'k0': 4.0E-2, 'k1': 3.0E7, 'k2': 1.0E4}
     # Define standard initial state and time range for instantiation
    uI = np.array([1.0, 0.0, 0.0]) # Standard initial condition for Robertson
    tI = 0.0
    tF = 1.0 # Arbitrary end time just for instantiation

    # Instantiate the RobertsonIVP class
    ivp = RobertsonIVP(uI, tI, tF, p)

    # Call the calc_all method 
    
    f, f_u = ivp.calc_all(u, True, True)
    #### END SOLUTION #####
...
def plot_Y(t, u, ptitle='Y vs t'):
    ...
    y_labels = ['Y0', 'Y1', 'Y2']
    y_lims = [(0., 1.), (0., 4.0e-5), (0., 1.)]
    for index, ax in enumerate(axs):
        ax.plot(t, u[:, index])
        ax.set_ylabel(y_labels[index])
        ax.set_ylim(y_lims[index])
        ax.grid(True)
...
def run_robertson_FE_stable():
    ...
    return run_robertson(mname='FE', dt=3.5e-4)
    #### END SOLUTION #####
