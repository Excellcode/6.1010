...
def J1(xv, pdict):
    ...
    """
    #### BEGIN SOLUTION ####
    x_sub = xv[0] - pdict['user'][0]
    y_sub = xv[1] - pdict['user'][1]
    D = 1 + (x_sub)**2 + (y_sub)**2
    J1 = -1/D
    J1prime = np.zeros(2)
    J1prime[0] = 2*x_sub/D**2
    J1prime[1] = 2*y_sub/D**2
    return J1, J1prime
    #### END SOLUTION ####
...
def run_test1():
    ...
    for i in range(Nx):
        for j in range(Ny):
            var[0], var[1] = bx[i], by[j]
            f[j, i] = J1(var, pdict)[0] 
    ...
    axs.plot(xhist[:-1, 0], xhist[:-1, 1], marker ='o', 
             linestyle ='None', color = 'green')
    axs.plot(xhist[-1, 0], xhist[-1, 1], 
             marker='o',         
             color='magenta')
    axs.grid(True)
    axs.axis('square')
