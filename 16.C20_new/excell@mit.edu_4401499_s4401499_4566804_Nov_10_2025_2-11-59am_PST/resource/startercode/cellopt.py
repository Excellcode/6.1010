...
def plot_objfun_onebase(axs, pdict, Nx=101, Ny=101):
    ...
    for j, y in enumerate(by):
        for i, x in enumerate(bx):
            f[j, i] = calc_objfun(np.array([x, y]), pdict)[0]
            
    cs = axs.contour(bx, by, f)
    min_val = np.argmin(f)
    j, i = np.unravel_index(min_val, f.shape)
    min_x = bx[i]
    min_y = by[j]
    min_f = f[j, i]
    axs.plot(min_x, min_y, marker = '*', color = 'red')
    axs.set_title(f"min f = {min_f:.2e} at ({min_x:.2e}, {min_y:.2e})")
    axs.clabel(cs)
    return cs
    #### END SOLUTION ####
...
def plot_optimize(axs, bases0, alpha, nStop, verbose=False, pdict={}):
    ...
    bhist, Jhist = gd.gradient_descent(calc_objfun, bases0v, alpha, nStop, False, pdict)
    Jmin = Jhist[-1]
    bmin = bhist[-1, :]
