...
with open(os.path.join(TEST_DIRECTORY, "test_recipes", "example_recipes.pickle"), "rb") as f:
...
def test_combine_recipes_small():
    ...
    # combine two with overlapping ingredients
    inp = [[{"a": 1}], [{"a": 1, "b": 2}]]
    inp2 = [[{"a": 1}], [{"a": 1, "b": 2}]]
    result = lab.combine_recipes(inp)
    ...
    # combine two with multiple possibilities
    inp = [[{"a1": 1}, {"a2": 1}], [{"b1": 1}, {"b2": 1}]]
    inp2 = [[{"a1": 1}, {"a2": 1}], [{"b1": 1}, {"b2": 1}]]
    result = lab.combine_recipes(inp)

    assert inp == inp2, "Be careful not to modify the input!"
    expected = [{"a1": 1, "b1": 1}, {"a1": 1, "b2": 1},
                {"a2": 1, "b1": 1}, {"a2": 1, "b2": 1},]
    ...
    result = lab.combine_recipes(inp)
    expected = [{"a": 9}, {"a": 6}, {"a": 10}, {"a": 5}]
    assert inp == inp2, "Be careful not to modify the input!"
    assert canonize_flat_recipes(result) == canonize_flat_recipes(expected)

    inp = [
    [{'peanut': 10}, {'almond': 10}],  # flat recipes for nut butter
    [{'grape': 20, 'sugar': 10}, {'strawberry': 5, 'sugar': 10}], # flat recipes for jelly
    [{'flour': 6, 'sugar': 2}] # flat recipes for bread
    ]
    inp2 = copy.deepcopy(inp)
    result = lab.combine_recipes(inp)
    expected = [
        {'peanut': 10, 'grape': 20, 'sugar': 12, 'flour': 6},
        {'peanut': 10, 'strawberry': 5, 'sugar': 12, 'flour': 6},
        {'almond': 10, 'grape': 20, 'sugar': 12, 'flour': 6},
        {'almond': 10, 'strawberry': 5, 'sugar': 12, 'flour': 6}
    ]
    assert inp == inp2, "Be careful not to modify the input!"
    assert canonize_flat_recipes(result) == canonize_flat_recipes(expected)
