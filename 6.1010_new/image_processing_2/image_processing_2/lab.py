...
def set_pixel(image, row, col, color):
    """changes the pixel at a location
    specified by 'row' and 'col' in 'image' to 'color'"""
...
def get_pixel_extend(image, row, col, boundary_behavior="zero"):
    """Returns a number representing the pixel
    in the intersection of specified row and column of an image array.

    Args:
        image: a dictionary representing the image
        row: the row of the pixel we are trying to access
        col: the column of the pixel we are trying to access
        boundary_behavior: determines a pixel which
        we can't access directly by specifying how we approach edge effects
    """
...
def apply_kernel_pixel(image, kernel, row, col, boundary_behavior="extend"):
    """helper function: returns the result of applying
    'kernel' to a pixel specified by a 'row' and 'col' in 'image',
    acceses pixel in image based on 'boundary_behavior'"""
    ...
    def output_func(image):
        ...
        return {
            ...
            "pixels": [
                (
                    output_red["pixels"][i],
                    output_green["pixels"][i],
                    output_blue["pixels"][i],
                )
...
def greyscale_image_from_color_image(color_image):
    ...
    return {
        ...
        "pixels": [
            round(0.299 * item[0] + 0.587 * item[1] + 0.114 * item[2])
            for item in color_image["pixels"]
        ],
    }
...
def custom_feature(image, color, x, y, radius):
    """
    draws a circle of the given radius with defined color
    at the given location centred at row,col = x, y
...
if __name__ == "__main__":
    ...
    # save_color_image(color_filter_from_greyscale_filter
    # (make_blur_filter(9))(load_color_image('test_images/python.png')), "python.png")
    # save_color_image(color_filter_from_greyscale_filter
    # (make_sharpen_filter(7))(load_color_image
    # ('test_images/sparrowchick.png')), "sparrowchick.png")
    # filter1 = color_filter_from_greyscale_filter(edges)
    # filter2 = color_filter_from_greyscale_filter(make_blur_filter(5))
    # filt = filter_cascade([filter1, filter1, filter2, filter1])
    # save_color_image(seam_carving((load_color_image
    # ("test_images/twocats.png")), 100), "cats.png")
    save_color_image(
        custom_feature(
            load_color_image("test_images/twocats.png"), (255, 0, 0), 35, 50, 20
        ),
        "cats_custom.png",
    )
