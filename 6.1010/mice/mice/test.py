...
def _test_bed_2d(test_number):
    ...
    with SpoofedPRNG(mice):
        game = lab.new_game_2d(nrows, ncols, nmice)
        ...
        for i, move in enumerate(moves):
            ...
            assert passed, format_error_message(seed, nrows, ncols, mice, moves_so_far, msg)
            render = lab.render_2d(game, all_visible=False)
            assert render == renders[i], (
                compare_renders(render, renders[i], False)
                + "\n\n"
                + format_error_message(seed, nrows, ncols, mice, moves_so_far, "")
            )
