...
def test_make_expression_additional():
    # 1. Multi-letter variables + mixed cases
    result = lab.make_expression("(Alpha + beta)")
    expected = lab.Add(lab.Var("Alpha"), lab.Var("beta"))
    assert expression_rep(result) == expression_rep(expected)

    # 2. Negative numbers inside deeper parentheses
    result = lab.make_expression("((x * -3.5) + (-2.1 * y))")
    expected = lab.Add(
        lab.Mul(lab.Var("x"), lab.Num(-3.5)),
        lab.Mul(lab.Num(-2.1), lab.Var("y"))
    )
    assert expression_rep(result) == expression_rep(expected)

    # 3. Very nested structure
    result = lab.make_expression("(((x + y) * (a - b)) / (c + (d * e)))")
    expected = lab.Div(
        lab.Mul(
            lab.Add(lab.Var("x"), lab.Var("y")),
            lab.Sub(lab.Var("a"), lab.Var("b"))
        ),
        lab.Add(lab.Var("c"), lab.Mul(lab.Var("d"), lab.Var("e")))
    )
    assert expression_rep(result) == expression_rep(expected)

    # 4. Leading negative variable expression
    result = lab.make_expression("(-x + 3)")
    expected = lab.Add(lab.Var("-x"), lab.Num(3))
    assert expression_rep(result) == expression_rep(expected)

    # 5. Mixed floats and ints
    result = lab.make_expression("((3.0 * x) - (y / 2))")
    expected = lab.Sub(
        lab.Mul(lab.Num(3.0), lab.Var("x")),
        lab.Div(lab.Var("y"), lab.Num(2))
    )
    assert expression_rep(result) == expression_rep(expected)

    # 6. Multiple consecutive parentheses
    result = lab.make_expression("(((x)))")
    expected = lab.Var("x")
    assert expression_rep(result) == expression_rep(expected)

    # 7. Expression with no spaces
    result = lab.make_expression("(x+y)")
    expected = lab.Add(lab.Var("x"), lab.Var("y"))
    assert expression_rep(result) == expression_rep(expected)

    # 8. Expression with excessive spaces
    result = lab.make_expression("(   x     *     (   y   + 3   )   )")
    expected = lab.Mul(
        lab.Var("x"),
        lab.Add(lab.Var("y"), lab.Num(3))
    )
    assert expression_rep(result) == expression_rep(expected)

    # 9. Unary minus inside nested layers
    result = lab.make_expression("((x + -2) / (y + -3))")
    expected = lab.Div(
        lab.Add(lab.Var("x"), lab.Num(-2)),
        lab.Add(lab.Var("y"), lab.Num(-3)),
    )
    assert expression_rep(result) == expression_rep(expected)

    # 10. Very long variable names
    result = lab.make_expression("(variable123 + another_var_456)")
    expected = lab.Add(lab.Var("variable123"), lab.Var("another_var_456"))
    assert expression_rep(result) == expression_rep(expected)
